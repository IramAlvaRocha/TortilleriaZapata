<?php
    ...
    $a_mes=$_POST['a_mes'];
    $a_ano=$_POST['a_ano'];
    $_SESSION['mes1_gasto']=$de_mes;
    $_SESSION['ano1_gasto']=$de_ano;
    $_SESSION['mes2_gasto']=$a_mes;
    $_SESSION['ano2_gasto']=$a_ano;
    $suc_checked=0;
    $zona_checked=0;
    $est_checked=0;
    ...
    $select="Select * from gasto where ID_sucursal in (";
    $where="";
    $i=1;
    $checks=0;
    ...
    if($where!=NULL){
        /* if($fecha_a!=Null && $fecha_de!=NULL){
            $where=$where . ") AND fecha_gasto BETWEEN '$fecha_de' AND '$fecha_a' ORDER BY ID_sucursal ASC;";
        }else{
        ...
        if($de_mes!='0' && $de_ano!='0' && $a_mes!='0' && $a_ano!='0'){
            $from=$where . ") AND MONTH(fecha_gasto)=$de_mes AND YEAR(fecha_gasto)=$de_ano ORDER BY ID_sucursal ASC;";
            $to=$where . ") AND MONTH(fecha_gasto)=$a_mes AND YEAR(fecha_gasto)=$a_ano ORDER BY ID_sucursal ASC;";
        }else{
    ...
    if($where==""){
        ...
        if($where!=NULL){
            /* if($fecha_a!='' && $fecha_de!=''){
                $where=$where . ") AND fecha_gasto BETWEEN '$fecha_de' AND '$fecha_a';";
            }else{
                $where=$where . ");";
            }
            ...
            if($de_mes!='0' && $de_ano!='0' && $a_mes!='0' && $a_ano!='0'){
                $from=$where . ") AND MONTH(fecha_gasto)=$de_mes AND YEAR(fecha_gasto)=$de_ano;";
                $to=$where . ") AND MONTH(fecha_gasto)=$a_mes AND YEAR(fecha_gasto)=$a_ano;";
            }else{
                $from=$where . ");";
                $to=$where . ");";
            }
    ...
    if($where==NULL){
        if($de_mes!='0' && $de_año!='0' && $a_mes!='0' && $a_año!='0'){
            ...
            $sql="Select * from gasto ORDER BY ID_sucursal ASC;";
            $sql2="";
        }
    }
...
<form name="form_result" action="Reporte_gastos_filtrado.php" method="POST">

...
require('../../../scripts/conexion.php');
					$conexion=conectar();                   
	$sql=$_SESSION['ventas_sql'];
    $sql2=$_SESSION['ventas_sql2'];
	$mes='';
	$ano1='';
	if($sql!=$sql2 && $sql2!=""){
		$mes=" en ";
		$mes1=$_SESSION['mes2'];
		$ano1=$ano1 . ' de ' . $_SESSION['ano2'];
		switch($mes1){
			...
			datasets : [
				{
					...
					data : [
						...
						while($lista=mysqli_fetch_array($resultado,MYSQLI_ASSOC)){
							...
							$resultado2=mysqli_query($conexion,$_SESSION['ventas_sql2']);
							$contador_venta=0;
							while($lista2=mysqli_fetch_array($resultado2,MYSQLI_ASSOC)){
								if($lista2['sucursal_venta']==$sucursal){

<?php
    ...
    session_start();
    $sql=$_POST['sql'];
    $_SESSION['gastossql']=$sql;
    $sql2=$_POST['sql2'];
    $_SESSION['gastossql2']=$sql2;
...
<?php
    ...
    while($lista1=mysqli_fetch_array($resultado1,MYSQLI_ASSOC)){
        ...
        echo'<tr>';
        echo'<td>' . $contador . '</td>';
        echo'<td>' . $lista1['idGasto'] . '</td>';
        echo'<td>' . $lista1['ID_sucursal'] . ' ' . $lista1['sucursal_gasto'] . '</td>';
        echo'<td>' . $lista1['descripcion_gasto'] . '</td>';
        echo'<td>' . $lista1['fecha_gasto'] . '</td>';
        echo'<td>' . $lista1['monto_gasto'] . '</td>';
...
<embed src="libreriasJS/chartJS/samples/grafica_gastos_filtrada.php" type="application/pdf" width="100%" height="500px" />
<?php 
    if($sql2!="" && $sql2!=$sql){
        echo '<embed src="libreriasJS/chartJS/samples/grafica_2_gastos_filtrada.php" type="application/pdf" width="100%" height="500px" />
        ';
    }
?>
</div></div></div>
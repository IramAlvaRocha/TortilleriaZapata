<?php
    ...
    $select="Select * from venta where ID_sucursal in (";
    $where="";
    $i=1;
    $checks=0;
    ...
    if($where!=NULL){
        ...
        if($de_mes!='0' && $de_ano!='0' && $a_mes!='0' && $a_ano!='0'){
            $from=$where . ") AND MONTH(fecha_Venta)=$de_mes AND YEAR(fecha_Venta)=$de_ano ORDER BY ID_sucursal ASC;";
            $to=$where . ") AND MONTH(fecha_Venta)=$a_mes AND YEAR(fecha_Venta)=$a_ano ORDER BY ID_sucursal ASC;";
        }else{
    ...
    if($where==""){
        ...
        if($where!=NULL){
            ...
            if($de_mes!='0' && $de_ano!='0' && $a_mes!='0' && $a_ano!='0'){
                $from=$where . ") AND MONTH(fecha_Venta)=$de_mes AND YEAR(fecha_Venta)=$de_ano;";
                $to=$where . ") AND MONTH(fecha_Venta)=$a_mes AND YEAR(fecha_Venta)=$a_ano;";
            }else{
                $from=$where . ");";
                $to=$where . ");";
            }
    ...
    if($where==NULL){
        if($de_mes!='0' && $de_año!='0' && $a_mes!='0' && $a_año!='0'){
            $sql="Select * from venta where MONTH(fecha_Venta)=$de_mes AND YEAR(fecha_Venta)=$de_ano ORDER BY ID_sucursal ASC; ";
            $sql2="Select * from venta where MONTH(fecha_Venta)=$a_mes AND YEAR(fecha_Venta)=$a_ano ORDER BY ID_sucursal ASC; ";
        }else{
            $sql="Select * from venta ORDER BY ID_sucursal ASC;";
            $sql2="";
        }
    }
...
<form name="form_result" action="Reporte_ventas_filtrado.php" method="POST">
...
if($de_mes!='0' && $de_ano!='0' && $a_mes!='0' && $a_ano!='0'){
                $date_from=$where . ") AND MONTH(fecha_Venta)=$de_mes AND YEAR(fecha_Venta)=$de_ano;";
                $date_to=$where . ") AND MONTH(fecha_Venta)=$a_mes AND YEAR(fecha_Venta)=$a_ano;";
            }else{
                $date_from=$where . ");";
                $date_to=$where . ");";
            }

            $sql1=$select . $date_from;
            $sql2=$select . $date_to;